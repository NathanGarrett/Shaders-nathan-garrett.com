//All of this is still from the shader module, and still isn't mine. 
...
void Shader::compileShader(const char * fileName,GLSLShader::GLSLShaderType type)
	throw(GLSLProgramException)
{
...
void Shader::compileShader(const std::string & source,GLSLShader::GLSLShaderType type,const char * fileName)
	throw(GLSLProgramException)
{
	if (handle <= 0) {
		handle = glCreateProgram();
...
void Shader::validate() throw(GLSLProgramException)
{
	if (!isLinked())
